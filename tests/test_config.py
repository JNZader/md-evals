...
"""
    ...
    def test_validate_valid_config(self):
        ...
        config = EvalConfig(
            name="Test",
            treatments={"CONTROL": Treatment(), "WITH_SKILL": Treatment(skill_path="./skill.md")},
    ...
    def test_validate_missing_control(self):
        ...
        config = EvalConfig(
            name="Test", treatments={"WITH_SKILL": Treatment(skill_path="./skill.md")}
        )
    ...
    def test_expand_wildcards_question(self):
        ...
        available = {"CONTROL": Treatment(), "TEST_A": Treatment(), "TEST_B": Treatment()}
    ...
    def test_save_config(self, tmp_path):
        ...
        config = EvalConfig(
            name="Test", defaults=Defaults(model="gpt-4o"), treatments={"CONTROL": Treatment()}
        )
...
class TestConfigErrorHandlingMutations:
    ...
    def test_unknown_treatment_raises_error(self):
        ...
        """
        EvalConfig(name="Test", treatments={"CONTROL": Treatment()})
...
class TestConfigLoadingMutations:
    ...
    def test_config_type_conversion_mutation(self):
        ...
        config = EvalConfig(
            name="TestConfig",
            version="2.0",
            defaults=Defaults(temperature=0.8, max_tokens=4096, timeout=120, retry_attempts=5),
        )
    ...
    def test_max_tokens_boundary_mutation(self):
        ...
        config1 = EvalConfig(name="Test", defaults=Defaults(max_tokens=1))
        assert config1.defaults.max_tokens == 1

        # Valid: large value
        config2 = EvalConfig(name="Test", defaults=Defaults(max_tokens=128000))
        ...
        config3 = EvalConfig(name="Test", defaults=Defaults(max_tokens=0))
    ...
    def test_timeout_validation_mutation(self):
        ...
        """
        # Valid: small timeout
        config1 = EvalConfig(name="Test", defaults=Defaults(timeout=1))
        assert config1.defaults.timeout == 1

        # Valid: normal timeout
        config2 = EvalConfig(name="Test", defaults=Defaults(timeout=60))
        assert config2.defaults.timeout == 60

        # Valid: large timeout
        config3 = EvalConfig(name="Test", defaults=Defaults(timeout=3600))
    ...
    def test_temperature_range_validation_mutation(self):
        ...
        config1 = EvalConfig(name="Test", defaults=Defaults(temperature=0.0))
        assert config1.defaults.temperature == 0.0

        # Valid: middle temperature
        config2 = EvalConfig(name="Test", defaults=Defaults(temperature=0.7))
        assert config2.defaults.temperature == 0.7

        # Valid: maximum temperature
        config3 = EvalConfig(name="Test", defaults=Defaults(temperature=2.0))
    ...
    def test_retry_attempts_fallback_mutation(self):
        ...
        """
        # Explicit value
        config1 = EvalConfig(name="Test", defaults=Defaults(retry_attempts=5))
...
class TestConfigValidationProperties:
    ...
    def test_config_type_invariants(self, retry_attempts, timeout):
        ...
        defaults = Defaults(
            retry_attempts=retry_attempts, timeout=timeout, temperature=0.7, max_tokens=2048
        )
    ...
    def test_config_boundary_values_accepted(
        self, retry_attempts, timeout, temperature, max_tokens
    ):
    ...
    def test_config_default_values_initialized(self, retry_attempts, model, provider):
        ...
        defaults = Defaults(retry_attempts=retry_attempts, model=model, provider=provider)
        config = EvalConfig(name="test", defaults=defaults)
    ...
    def test_config_collections_preserved(self, treatment_name, test_name):
        ...
        treatments = {
            treatment_name: Treatment(description=f"Test {treatment_name}", skill_path=None)
        }
        tests = [Task(name=test_name, description="Test", prompt="test")]

        config = EvalConfig(name="test", treatments=treatments, tests=tests)
...
class TestConfigEdgeCases:
    ...
    @given(version=st.sampled_from(["1.0", "2.0", "1.5", "0.1"]))
    ...
    def test_config_model_provider_consistency(self, model, provider):
        ...
        assert config.defaults.provider == provider, (
            f"Provider changed: {provider} -> {config.defaults.provider}"
        )
...
""")
    ...
    def test_cost_map_direct_construction(self):
        ...
        config = EvalConfig(
            name="Test",
            cost_map={"gpt-4o": {"input_rate_per_million": 5.0, "output_rate_per_million": 15.0}},
        )
