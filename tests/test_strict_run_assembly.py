...
def record(kind="structure", identifier=None):
    return {
        "id": identifier or (ID if kind == "structure" else "e" * 64),
        "kind": kind,
        "repository": "repo",
        "revision": REVISION,
...
def declarations():
    ...
    for name, label in (("one", "Answer A"), ("two", "Answer B"), ("three", "Answer C")):
        ...
        references[name] = {
            "reference_digest": digest(reference_payload),
            "reference_label": label,
            "annotation_policy": "approved annotation policy",
            "reference_payload": reference_payload,
        }
    gold_payload = {
        "version": "gold-v1",
        "source_artifact": {
            "schema": "gold-source.v1",
            "source_id": "fixture-1",
            "artifact_digest": digest(source_payload),
    ...
    billing_scaffold = build_billing_scaffold(
        ...
        planned_cells=[
            (case["name"], arm["arm"]) for case in plan().to_dict()["cases"] for arm in case["arms"]
        ],
    ...
    return {
        "gold_reference": {
            **gold_payload,
            "digest": hashlib.sha256(
                json.dumps(
                    gold_payload, sort_keys=True, separators=(",", ":"), ensure_ascii=True
                ).encode()
            ).hexdigest(),
        },
        "reviewer_metadata": {
        ...
        "cleanup_declaration": {
            "retention_policy": "delete raw artifacts after deadline",
...
@pytest.mark.parametrize(
    "policy",
    [
        {"adapter_retries": 1, "fallbacks": False},
...
def test_gold_digest_mismatch_and_weak_reference_rejected():
    ...
    gold = {
        **declarations()["gold_reference"],
        "per_task_references": {
            **declarations()["gold_reference"]["per_task_references"],
            "one": {},
        },
    }
...
def test_billing_evidence_is_payload_bound_and_provider_bound():
    ...
    with pytest.raises(StrictRunAssemblyError, match="evidence digest"):
        finalize_strict_run_assembly(
            assemble(), assemble().sha256, provider_billing_attestation=billing
        )
...
def test_repo_cleanup_consent_and_secret_fail_closed():
    ...
    with pytest.raises(StrictRunAssemblyError):
        assemble(
            cleanup_declaration={
                **declarations()["cleanup_declaration"],
    ...
    with pytest.raises(StrictRunAssemblyError):
        assemble(
            repository_state={**declarations()["repository_state"], "revision": "Bearer secret"}
        )
...
def test_mutation_extra_fields_and_execution_boundaries():
    ...
    values["gold_reference"]["per_task_references"]["one"]["reference_payload"]["answer"] = (
        "MUTATED"
    )
...
def test_parser_validates_complete_schema_before_returning_assembly():
    ...
    with pytest.raises(StrictRunAssemblyError):
        parse_strict_run_assembly(
            json.dumps({"profile": "StrictRunAssembly.v2", "state": "candidate"})
        )
...
def test_boolean_repetitions_is_rejected_after_rehash():
    ...
    with pytest.raises(
        StrictRunAssemblyError, match="permanently blocked or has invalid repetitions"
    ):
        build_strict_run_assembly(plan=rehashed_plan_manifest(manifest), **declarations())
...
def _rehash_assembly(manifest, *, plan_sha256=None):
    ...
    return assembly_type(
        ...
        plan_sha256 or assemble().plan_sha256,
...
def test_finalizer_rejects_self_hashed_fabricated_minimal_candidate():
    raw = json.dumps(
        {"profile": "StrictRunAssembly.v2", "state": "candidate", "execution_authorized": False},
        sort_keys=True,
        separators=(",", ":"),
    )
    with pytest.raises(StrictRunAssemblyError, match="StrictRunAssembly must be created"):
...
def scoped_dirty_repository(**changes):
    payload = {
        ...
        "relevant_files": [
            {"path": "md_evals/example.py", "git_status": "??", "bytes": 7, "sha256": "e" * 64}
        ],
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda value: value.update(
            relevant_files=[
                {"path": "/absolute", "git_status": "??", "bytes": 1, "sha256": "e" * 64}
            ]
        ),
        lambda value: value.update(
            relevant_files=[
                {"path": "../escape", "git_status": "??", "bytes": 1, "sha256": "e" * 64}
            ]
        ),
        lambda value: value.update(
            relevant_files=[{"path": "file", "git_status": "??", "bytes": True, "sha256": "e" * 64}]
        ),
        lambda value: value.update(
            relevant_files=[{"path": "file", "git_status": "??", "bytes": 1, "sha256": "E" * 64}]
        ),
    ],
)
