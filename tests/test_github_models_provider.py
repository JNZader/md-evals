...
class TestProviderInitialization:
    ...
    def test_init_with_valid_token_from_env(self, valid_token_env, mock_azure_client):
        ...
        with patch.object(
            GitHubModelsProvider, "_initialize_client", return_value=mock_azure_client
        ):
            provider = GitHubModelsProvider("claude-3.5-sonnet")
    ...
    def test_init_with_custom_timeout(self, valid_token_env, mock_azure_client):
        ...
        with patch.object(
            ...
            provider = GitHubModelsProvider("claude-3.5-sonnet", timeout_seconds=120)
    ...
    def test_init_with_custom_retries(self, valid_token_env, mock_azure_client):
        ...
        with patch.object(
            ...
            provider = GitHubModelsProvider("claude-3.5-sonnet", max_retries=5)
...
class TestTokenLoading:
    ...
    def test_load_token_from_env(self, valid_token_env):
        ...
        token = GitHubModelsProvider("claude-3.5-sonnet")._load_github_token()
    ...
    def test_load_token_explicit_overrides_env(self, valid_token_env):
        ...
        provider = GitHubModelsProvider("claude-3.5-sonnet", github_token=explicit_token)
    ...
    def test_load_token_from_gh_cli_fallback(self, no_token_env, mock_azure_client):
        ...
        with (
            patch.object(GitHubModelsProvider, "_load_token_from_gh_cli", return_value=gh_token),
            patch.object(
                GitHubModelsProvider, "_initialize_client", return_value=mock_azure_client
            ),
        ):
    ...
    def test_resolve_token_source_prefers_env_over_gh(self, monkeypatch):
        ...
        with patch.object(
            GitHubModelsProvider, "_load_token_from_gh_cli", return_value="gho_other"
        ):
...
class TestProviderRegistry:
    ...
    def test_instantiate_from_registry(self, valid_token_env, mock_azure_client):
        ...
        with patch.object(
            ...
            provider = ProviderRegistry.instantiate("github-models", "claude-3.5-sonnet")
...
class TestLLMResponseIntegration:
    ...
    async def test_complete_returns_llm_response(
        self, provider_with_mock_client, mock_azure_client
    ):
        ...
        with patch.object(
            provider_with_mock_client, "_stream_completion", new_callable=AsyncMock
        ) as mock_stream:
    ...
    async def test_complete_with_temperature(self, provider_with_mock_client, mock_azure_client):
        ...
        with patch.object(
            ...
            response = await provider_with_mock_client.complete("Test", temperature=0.5)
...
class TestErrorPathsCoverage:
    ...
    @pytest.mark.asyncio
    async def test_complete_with_rate_limit_error(
        self, provider_with_mock_client, mock_azure_client
    ):
        """Test rate limit error detection and handling."""
        # Simulate rate limit error (429 status code)
        mock_azure_client.complete.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_complete_with_context_window_error(
        self, provider_with_mock_client, mock_azure_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_complete_with_authentication_error(
        self, provider_with_mock_client, mock_azure_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_complete_with_generic_api_error(
        self, provider_with_mock_client, mock_azure_client
    ):
...
class TestStreamingErrorHandling:
    ...
    async def test_handle_stream_with_no_usage_data(self, provider_with_mock_client):
        ...
        response = Mock(choices=[Mock(message=Mock(content="Test content"))], usage=None)
    ...
    async def test_handle_stream_with_empty_content(self, provider_with_mock_client):
        ...
        response = Mock(choices=[Mock(message=Mock(content=""))], usage=Mock(completion_tokens=0))
...
class TestResponseFormats:
    ...
    @pytest.mark.asyncio
    async def test_complete_with_estimated_tokens(
        self, provider_with_mock_client, mock_azure_client
    ):
        """Test response with estimated tokens (no usage data)."""
        mock_azure_client.complete.return_value = Mock(
            choices=[Mock(message=Mock(content="Response without usage"))], usage=None
        )
...
class TestProviderInitializationErrors:
    ...
    def test_unsupported_model_error_lists_supported(self):
        ...
        assert (
            "gpt" in error_msg.lower()
            or "claude" in error_msg.lower()
        ...
        with patch.dict(os.environ, {"GITHUB_TOKEN": "test_token"}):
            ...
            with patch.object(provider, "_stream_completion") as mock_stream:
                ...
                assert (
                    ...
                    or "streaming" in error_msg.lower()
                )
...
class TestGitHubModelsNumericProperties:
    ...
    @given(text_length=st.integers(min_value=0, max_value=10000))
    ...
    @given(text_length=st.integers(min_value=1, max_value=5000))
    @settings(suppress_health_check=[HealthCheck.filter_too_much])
    ...
    def test_token_estimation_consistency(self, text_length):
        ...
        assert tokens1 == tokens2 == tokens3, (
            f"Token estimation should be deterministic: {tokens1}, {tokens2}, {tokens3}"
        )
    ...
    def test_token_estimation_no_overflow(self, text_length):
        ...
        assert estimated <= expected_max, (
            f"Token count unreasonably high: {text_length} chars → {estimated} tokens"
        )

    @given(text_length=st.integers(min_value=1, max_value=1000))
    @settings(suppress_health_check=[HealthCheck.filter_too_much])
