...
class Reporter:
    ...
    def report_terminal(self, results: list[ExecutionResult], verbose: bool = False) -> None:
        ...
        # Build summary table
        table = Table(
            title="md-evals Results", box=box.ROUNDED, show_header=True, header_style="bold magenta"
        )
        ...
        if "CONTROL" in by_treatment:
            ...
            for treatment, treatment_results in by_treatment.items():
                ...
                treatment_rate = (
                    (treatment_passed / treatment_total * 100) if treatment_total > 0 else 0
                )
                ...
                if improvement > 0:
                    ...
                    self.console.print(f"[red]▼[/red] {treatment}: {improvement:.0f}% vs CONTROL")
    ...
    def report_regression_delta(self, findings: "list[RegressionItem]") -> None:
    ...
    def _print_verbose(self, results: list[ExecutionResult]) -> None:
        ...
        for result in results:
            panel = Panel(
                ...
                f"{result.response.duration_ms if result.response else 'N/A'}ms",
                title=f"Result: {result.treatment}/{result.test}",
                border_style="blue" if result.passed else "red",
            )
            self.console.print(panel)

    def report_json(self, results: list[ExecutionResult], output_path: str) -> None:
    ...
    def report_markdown(self, results: list[ExecutionResult], output_path: str) -> None:
    ...
    def _build_output_data(self, results: list[ExecutionResult]) -> dict[str, Any]:
        ...
        output: dict[str, Any] = {
            ...
            "results": [self._build_result_item(r) for r in results],
            "summary": summary,
        }
            ...
            for result in treatment_results:
                ...
                if result.evaluator_results:
                    ...
                    for eval_result in result.evaluator_results:
                        ...
                        md_lines.append(f"    - {eval_result.evaluator_name}: {eval_status}")

            md_lines.append("")

        return "\n".join(md_lines)

    def calculate_summary(self, results: list[ExecutionResult]) -> dict[str, Any]:
