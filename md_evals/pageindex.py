...
class DocumentPageIndex:
    ...
    def paginate_document(
        self, doc_id: str, content: str, max_tokens_per_page: int = 1500
    ) -> Dict[str, int]:
    ...
    def get_context(self, doc_id: str, page_num: int, window_size: int = 1) -> Dict[str, Any]:
    ...
    def store_evaluation(self, doc_id: str, page_num: int, result: Dict[str, Any]) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (doc_id, page_num, result.get("score", 0), json.dumps(result.get("issues", []))),
            )
            conn.commit()
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                pages.append(
                    {
                        "page_num": row[0],
                        "evaluation": {"score": row[1], "issues": json.loads(row[2] or "[]")},
    ...
    def check_model_fit(
        self, doc_id: str, model_tokens: int, safety_margin: float = 0.3
    ) -> Dict[str, Any]:
    ...
    def get_doc_stats(self, doc_id: str) -> DocumentStats:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            return DocumentStats(pages=pages, tokens=tokens, evaluated_pages=evaluated)
