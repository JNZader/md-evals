...
class EmptyFileRule(LinterRule):
    ...
    def check(self, skill_path: str, content: str) -> list[LinterViolation]:
        if not content.strip():
            return [
                LinterViolation(rule="empty-file", message="Skill file is empty", severity="error")
            ]
        return []
    ...
    def run(self, skill_path: str) -> LinterReport:
        ...
        try:
            ...
            return LinterReport(
                ...
                violations=[
                    LinterViolation(
                        rule="read-error", message=f"Error reading file: {e}", severity="error"
                    )
                ],
            )
        ...
        return LinterReport(
            skill_path=skill_path, passed=passed, violations=all_violations, line_count=line_count
        )
