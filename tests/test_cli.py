...
""".strip()
        ...
        result = runner.invoke(
            app, ["smoke", "--config", str(eval_file), "--provider", "github-models"]
        )
...
""".strip()
        ...
        with patch(
            "md_evals.providers.github_models.GitHubModelsProvider._load_token_from_gh_cli",
            return_value=None,
        ):
            runner = CliRunner()
            result = runner.invoke(
...
""")
        ...
        with patch("md_evals.cli.LLMAdapter"), patch("md_evals.cli.ExecutionEngine") as mock_engine:
            # Setup mock engine
            ...
            runner = CliRunner()
            result = runner.invoke(
                app, ["run", "--config", str(eval_file), "--model", "gpt-3.5-turbo", "--no-lint"]
            )
...
""")
        ...
        with patch("md_evals.cli.LLMAdapter"), patch("md_evals.cli.ExecutionEngine") as mock_engine:
            ...
            runner = CliRunner()
            result = runner.invoke(app, ["run", "--config", str(eval_file), "-n", "4", "--no-lint"])
...
""")
        ...
        with patch("md_evals.cli.LLMAdapter"), patch("md_evals.cli.ExecutionEngine") as mock_engine:
            ...
            runner = CliRunner()
            result = runner.invoke(
                app, ["run", "--config", str(eval_file), "--count", "3", "--no-lint"]
            )
...
""")
        ...
        with (
            ...
            runner = CliRunner()
            result = runner.invoke(
                app, ["run", "--config", str(eval_file), "--output", "json", "--no-lint"]
            )
...
""")
        ...
        with (
            ...
            runner = CliRunner()
            result = runner.invoke(
                app, ["run", "--config", str(eval_file), "--output", "markdown", "--no-lint"]
            )
...
""")
        ...
        runner = CliRunner()
        result = runner.invoke(app, ["run", "--config", str(eval_file)])
...
""")
        ...
        with (
            ...
            runner = CliRunner()
            result = runner.invoke(
                app, ["run", "--config", str(eval_file), "--verbose", "--no-lint"]
            )
...
""")
        ...
        with (
            ...
            runner = CliRunner()
            result = runner.invoke(
                app, ["run", "--config", str(eval_file), "--provider", "github-models", "--no-lint"]
            )
...
""")
        ...
        with (
            ...
            runner = CliRunner()
            result = runner.invoke(app, ["run", "--config", str(eval_file), "--no-lint"])
...
""")
        ...
        with (
            ...
            with caplog.at_level(logging.DEBUG):
                result = runner.invoke(
                    app, ["run", "--config", str(eval_file), "--debug", "--no-lint"]
                )
...
""")
        ...
        result = runner.invoke(
            app,
            ["run", "--config", str(eval_file), "--provider", "nonexistent-provider", "--no-lint"],
        )
...
""")
        ...
        with (
            ...
            result2 = ExecutionResult(
                ...
                prompt="test2",
                response=LLMResponse(
                    content="bye", model="gpt-4o", provider="openai", duration_ms=1000
                ),
                passed=False,
                evaluator_results=[],
                timestamp="2024-01-01T00:00:00",
...
""")
        ...
        with (
            ...
            runner = CliRunner()
            result = runner.invoke(
                app, ["run", "--config", str(eval_file), "--treatment", "WITH_SKILL", "--no-lint"]
            )
...
""")
        ...
        with patch("md_evals.cli.LLMAdapter") as mock_adapter:
            ...
            mock_adapter.side_effect = Exception(
                "GitHub API authentication failed: token not found or invalid"
            )
...
class TestCLIStringProcessingProperties:
    ...
    @given(text1=st.text(min_size=0, max_size=200), text2=st.text(min_size=0, max_size=200))
    ...
    @given(
        error_message=st.text(
            min_size=1, max_size=300, alphabet=st.characters(blacklist_categories=("Cc", "Cs"))
        )
    )
    def test_cli_sensitive_data_not_exposed_in_errors(self, error_message):
        ...
        sensitive_count = sum(
            1 for keyword in sensitive_keywords if keyword.lower() in message_lower
        )
...
class TestCLIEdgeCases:
    ...
    @given(whitespace_text=st.from_regex(r"[\s]+", fullmatch=True))
    def test_cli_handles_whitespace_strings(self, whitespace_text):
        ...
        assert (
            whitespace_text
            == whitespace_text.strip() + whitespace_text[len(whitespace_text.strip()) :]
        )
