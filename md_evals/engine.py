...
from md_evals.models import EvalConfig, ExecutionResult, Task, Treatment, LLMResponse
...
class ExecutionEngine:
    ...
    async def run_single(
        ...
        async with self._get_semaphore():
            ...
            final_prompt, system_prompt = inject_skill(task.prompt, treatment.skill_path)
            ...
            if self.evaluator_engine and task.evaluators:
                evaluator_results = await self.evaluator_engine.evaluate(
                    response.content, task.evaluators
                )
    ...
    async def run_all(
        ...
        for treatment_name in available_treatments:
            for task in self.config.tests:
                tasks_to_run.append((available_treatments[treatment_name], task, treatment_name))
                ...
                for i, result in enumerate(results):
                    if isinstance(result, Exception):
                        ...
                        error_result = self._make_error_result(
                            treatment, task, treatment_name, result
                        )
