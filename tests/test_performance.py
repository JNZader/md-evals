...
class TestRegexPerformance:
    ...
    def test_simple_pattern_match(self, benchmark):
        ...
        evaluator = RegexEvaluator(name="simple", pattern="test", pass_on_match=True)
    ...
    def test_multiline_pattern_large_text(self, benchmark):
        ...
        """
        engine = EvaluatorEngine()
        evaluator = RegexEvaluator(name="multiline", pattern="^line.*success$", pass_on_match=True)
    ...
    def test_pattern_compilation_cache_effect(self, benchmark):
        ...
        """
        engine = EvaluatorEngine()
        evaluator = RegexEvaluator(name="compiled", pattern=r"success|failed", pass_on_match=True)
...
class TestExactMatchPerformance:
    ...
    def test_exact_match_small_string(self, benchmark):
        ...
        """
        engine = EvaluatorEngine()
        evaluator = ExactMatchEvaluator(name="exact", expected="success", case_sensitive=True)
    ...
    def test_exact_match_large_string(self, benchmark):
        ...
        evaluator = ExactMatchEvaluator(
            name="exact", expected="NEEDLE_IN_HAYSTACK", case_sensitive=True
        )
        # 1MB of text
    ...
    def test_exact_match_case_insensitive(self, benchmark):
        ...
        """
        engine = EvaluatorEngine()
        evaluator = ExactMatchEvaluator(name="exact", expected="Success", case_sensitive=False)
    ...
    def test_json_report_large(self, benchmark, tmp_path):
        ...
        results = [
            ExecutionResult(
                ...
                evaluator_results=[
                    EvaluatorResult(
                        evaluator_name=f"evaluator_{j}", passed=True, score=0.95, reason=None
                    )
                    for j in range(3)
                ],
                timestamp="2026-03-11T10:00:00Z",
...
class TestAsyncConcurrencyPerformance:
    ...
    def test_concurrent_regex_evaluation(self, benchmark):
        ...
        """
        engine = EvaluatorEngine()
        evaluator = RegexEvaluator(name="test", pattern="success", pass_on_match=True)
...
class TestJSONParsingPerformance:
        ...
        json_str = json.dumps(
            {
                ...
                "suggestions": [f"suggestion_{i}: improve aspect {i}" for i in range(20)],
            }
        )
    ...
    def test_large_json_array_parse(self, benchmark):
        ...
        json_str = json.dumps(
            {
                "results": [
                    {"id": i, "value": f"result_{i}", "score": 0.5 + (i % 100) / 100}
                    for i in range(1000)
                ]
            }
        )
...
class TestEvaluatorEngineAsyncPerformance:
    ...
    def test_llm_judge_error_handling_performance(self, benchmark):
        ...
        def evaluate_with_error():
            ...
            regex_eval = RegexEvaluator(name="judge", pattern="test", pass_on_match=True)
...
class TestMemoryPatterns:
    ...
    def test_list_comprehension_vs_loop(self, benchmark):
        ...
        """
        evaluators = [
            RegexEvaluator(name=f"eval_{i}", pattern="test", pass_on_match=True) for i in range(100)
        ]

        def process_list():
            return [
                EvaluatorResult(evaluator_name=e.name, passed=True, score=1.0, reason=None)
                for e in evaluators
            ]
