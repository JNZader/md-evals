...
def _print_github_auth_help() -> None:
    ...
    console.print(
        "   [dim]md-evals smoke --provider github-models --config "
        "examples/eval_with_github_models.yaml[/dim]"
    )
...
def check(
    ...
    rubric: Annotated[
        Optional[str], typer.Option("--rubric", "-r", help="Path to rubric.yaml")
    ] = None,
    verbose: Annotated[
        bool, typer.Option("--verbose", "-v", help="Show detailed check results")
    ] = False,
):
    ...
    if result.passed:
        console.print(
            f"[green]✓ {skill_path} — Pre-check PASSED ("
            f"{result.checks_run} checks, {len(result.findings)} findings, "
            f"{result.duration_ms}ms)[/green]"
        )
    else:
        sum(1 for f in result.findings if f.severity == "error")
        sum(1 for f in result.findings if f.severity == "warning")
        console.print(
            f"[red]✗ {skill_path} — Pre-check FAILED ("
            f"{result.checks_run} checks, {len(result.findings)} findings, "
            f"{result.duration_ms}ms)[/red]"
        )
    ...
    if result.findings:
        for finding in result.findings:
            ...
            console.print(
                f"  [{severity_color}][{finding.severity.upper()}]"
                f"[/{severity_color}] {finding.message}{line_info}"
            )
...
def run(
    ...
    treatment: Annotated[
        Optional[str],
        typer.Option("--treatment", "-t", help="Treatment(s) to run (comma-separated or wildcard)"),
    ] = None,
    model: Annotated[Optional[str], typer.Option("--model", "-m", help="Override model")] = None,
    provider: Annotated[
        Optional[str],
        typer.Option(
            "--provider", "-p", help="Override provider (e.g., github-models, openai, anthropic)"
        ),
    ] = None,
    rubric: Annotated[
        Optional[str], typer.Option("--rubric", "-r", help="Path to rubric.yaml")
    ...
    no_pre_check: Annotated[
        bool, typer.Option("--no-pre-check", help="Skip pre-check phase")
    ] = False,
    force: Annotated[
        bool, typer.Option("--force", help="Run LLM eval even on pre-check errors")
    ] = False,
    verbose: Annotated[bool, typer.Option("--verbose", "-v", help="Verbose output")] = False,
    debug: Annotated[
        bool, typer.Option("--debug", help="Enable debug logging for provider initialization")
    ] = False,
    ...
    pipeline: Annotated[
        Optional[bool], typer.Option("--pipeline/--no-pipeline", help="Force pipeline mode on/off")
    ] = None,
    probe: Annotated[
        Optional[str],
        typer.Option("--probe", help="Comma-separated probe names (e.g., dimension,edge-case)"),
    ] = None,
    mode: Annotated[
        Optional[str], typer.Option("--mode", help="Testing mode: smoke, reliable, regression")
    ] = None,
    fail_fast: Annotated[
        Optional[bool], typer.Option("--fail-fast/--no-fail-fast", help="Stop on first failure")
    ] = None,
):
    ...
    if pipeline is True:
        ...
        config_obj.pipeline
        and isinstance(config_obj.pipeline, dict)
        and config_obj.pipeline.get("enabled", False)
    ):
        use_pipeline = True
    ...
    if use_pipeline:
        ...
        pipeline_config = PipelineConfig(
            **{k: v for k, v in pipeline_dict.items() if k != "enabled"}
        )
        ...
        runner = PipelineRunner(
            config=config_obj, rubric=rubric_config, pipeline_config=pipeline_config
        )
        ...
        console.print(
            f"  Overall Grade: [bold]{result.overall_grade}[/bold]  "
            f"Score: {result.overall_score:.2f}"
        )
        if result.dimensions:
            for dim in result.dimensions:
                console.print(
                    f"  {dim.dimension}: {dim.score:.2f} ({dim.grade}) weight={dim.weight:.2f}"
                )
    ...
    if not no_pre_check:
        ...
        if pre_check_failed and not force:
            console.print(
                "[red]Pre-check failed. Use --force to run LLM eval anyway, "
                "or --no-pre-check to skip.[/red]"
            )
            raise typer.Exit(code=2)
        elif pre_check_failed and force:
            console.print(
                "[yellow]Pre-check failed but --force specified. "
                "Continuing with LLM eval...[/yellow]"
            )

    # Run linter first (optional)
    if not no_lint:
        lint_config = LinterConfig(
            max_lines=config_obj.lint.max_lines, fail_on_violation=config_obj.lint.fail_on_violation
        )
    ...
    try:
        logger.debug(
            f"Initializing LLM adapter: provider={config_obj.defaults.provider}, "
            f"model={config_obj.defaults.model}"
        )
    ...
    engine = ExecutionEngine(
        config=config_obj, llm_adapter=llm_adapter, evaluator_engine=evaluator_engine
    )
    ...
    if mode_defaults is not None:
        ...
        if mode_defaults.compare_baseline:
            ...
            if baseline is None:
                console.print("[yellow]No previous baseline — saving current as baseline[/yellow]")
            else:
    ...
    has_regressions = regression_findings is not None and any(
        f.status == "regression" for f in regression_findings
    )
    if has_regressions:
...
def lint(
    ...
    fail: Annotated[
        bool, typer.Option("--fail", "-f", help="Exit with error on violations")
    ] = True,
...
def smoke(
    config: Annotated[
        str, typer.Option("--config", "-c", help="Config file path to validate")
    ] = "eval.yaml",
    provider: Annotated[
        str, typer.Option("--provider", "-p", help="Provider to preflight")
    ] = "github-models",
):
    ...
    if provider.lower().replace("_", "-") in {"github-models", "github models"}:
        ...
        if token:
            source_label = (
                "GITHUB_TOKEN" if source == "env" else "gh auth token" if source == "gh" else source
            )
...
def list_models(
    provider: Annotated[
        Optional[str],
        typer.Option("--provider", "-p", help="Provider to list models for (default: all)"),
    ] = None,
    verbose: Annotated[
        bool, typer.Option("--verbose", "-v", help="Show additional metadata")
    ] = False,
):
...
def _display_provider_models(
    provider_name: str, provider_class, models: dict, verbose: bool = False
):
    ...
    for model_name, metadata in models.items():
        if verbose:
            table.add_row(
                ...
                f"{getattr(metadata, 'temperature_range', (0, 1))[0]:.1f}–"
                f"{getattr(metadata, 'temperature_range', (0, 1))[1]:.1f}",
...
def suite_run(
    config: Annotated[
        Optional[str], typer.Option("--config", "-c", help="Suite YAML config file")
    ] = None,
    suite_name: Annotated[
        Optional[str], typer.Option("--suite", "-s", help="Named suite to run")
    ] = None,
    rubric: Annotated[
        Optional[str], typer.Option("--rubric", "-r", help="Rubric override path")
    ] = None,
    eval_config: Annotated[
        str, typer.Option("--eval-config", help="Eval config for model defaults")
    ] = "eval.yaml",
    verbose: Annotated[bool, typer.Option("--verbose", "-v", help="Verbose output")] = False,
):
    ...
    # Run suite
    console.print(
        f"[cyan]Running suite: {suite_config.name or 'unnamed'} "
        f"({len(suite_config.skills)} skills)[/cyan]"
    )
...
def eval_plugin_cmd(
    ...
    min_grade: Annotated[
        str, typer.Option("--min-grade", "-g", help="Minimum acceptable grade")
    ] = "C",
    ...
    export_html: Annotated[
        Optional[str], typer.Option("--export", help="Export HTML report to path")
    ] = None,
    verbose: Annotated[bool, typer.Option("--verbose", "-v", help="Verbose output")] = False,
):
    ...
    if min_grade not in GRADE_ORDER:
        console.print(
            f"[red]Invalid grade: {min_grade}. Must be one of {', '.join(GRADE_ORDER)}[/red]"
        )
        raise typer.Exit(code=1)
    ...
    console.print(f"  Aggregate: grade={result.aggregate_grade} score={result.aggregate_score:.2f}")
...
def analytics_trends(
    ...
    store_path: Annotated[
        str, typer.Option("--store", help="Analytics store path")
    ...
    console.print(
        f"\n  Latest: [{grade_colors.get(trend.latest_grade, 'white')}]"
        f"{trend.latest_grade}[/{grade_colors.get(trend.latest_grade, 'white')}]  "
        f"Trend: {icon} {trend.trend_direction}"
    )
...
mission_app = typer.Typer(
    name="mission", help="Run YAML-based mission test suites for regression tracking"
)
...
def mission_run(
    ...
    model: Annotated[Optional[str], typer.Option("--model", "-m", help="Override model")] = None,
    provider: Annotated[
        Optional[str], typer.Option("--provider", "-p", help="Override provider")
    ] = None,
    no_track: Annotated[bool, typer.Option("--no-track", help="Skip regression tracking")] = False,
    report: Annotated[
        Optional[str], typer.Option("--report", "-r", help="Save markdown report to path")
    ] = None,
    verbose: Annotated[bool, typer.Option("--verbose", "-v", help="Verbose output")] = False,
):
    ...
    console.print(
        f"[cyan]Running mission: {config.name} ({len(config.test_cases)} test cases)[/cyan]"
    )
    ...
    console.print()
    table = Table(
        title=f"Mission: {result.mission_name}", show_header=True, header_style="bold magenta"
    )
    ...
    console.print(
        ...
        f"({s.pass_rate:.0%}) in {s.duration_ms}ms"
    )
        ...
        if regression.has_regressions:
            ...
            for item in regression.items:
                if item.status == "regression":
                    console.print(
                        f"  [red]- {item.test_name}: PASS -> FAIL ({item.score_delta:+.2f})[/red]"
                    )
...
def mission_report(
    mission_file: Annotated[str, typer.Argument(help="Path to mission YAML file")],
    output: Annotated[
        str, typer.Option("--output", "-o", help="Output path for report")
    ] = "mission-report.md",
):
...
def dashboard(
    dashboard_file: Annotated[
        str, typer.Argument(help="Path to the Markdown dashboard file with sql-eval blocks")
    ],
    store: Annotated[
        str, typer.Option("--store", "-s", help="Path to the JSONL analytics store")
    ] = ".md-evals/analytics.jsonl",
    output: Annotated[
        Optional[str], typer.Option("--output", "-o", help="Output file path (defaults to stdout)")
    ] = None,
    db_path: Annotated[
        Optional[str],
        typer.Option("--db-path", help="Persist the SQLite DB to this path for reuse"),
    ] = None,
):
